<x-app-layout>
    ...
    <div class="py-8 px-4 sm:px-6 lg:px-8 min-h-screen">
        <div class="max-w-6xl mx-auto space-y-6">
            <div class="app-panel-strong rounded-3xl p-6">
                <form method="GET" action="{{ route('clinical_histories.index') }}" class="flex flex-col md:flex-row gap-3">
                    <input
                        ...
                        class="w-full md:flex-1 rounded-full border-slate-200 text-sm font-semibold text-slate-700 focus:border-teal-500 focus:ring-teal-500 dark:bg-slate-900/70"
                    >
                    <button type="submit" class="px-6 py-3 rounded-full bg-teal-700 text-white text-[10px] font-black uppercase tracking-[0.22em] hover:bg-teal-600 transition w-full md:w-auto">
                        Buscar
                    </button>
                    <a href="{{ route('clinical_histories.index') }}" class="px-6 py-3 rounded-full bg-slate-100 text-slate-600 text-[10px] font-black uppercase tracking-[0.22em] hover:bg-slate-200 transition text-center w-full md:w-auto dark:bg-slate-800 dark:text-slate-200">
                        Limpiar
                    </a>
                </form>
            </div>

            <div class="app-panel-strong rounded-3xl overflow-hidden">
                                ...
                                <tr class="hover:bg-slate-50 transition-colors">
                                    ...
                                    <td class="px-6 py-4 text-right">
                                        <div class="flex flex-col sm:flex-row justify-end gap-2">
                                            <a href="{{ route('clinical_histories.show', $student) }}" class="inline-flex items-center justify-center px-4 py-2 rounded-full bg-slate-900 text-white text-[11px] font-black uppercase tracking-widest hover:bg-teal-700 transition w-full sm:w-auto">
                                                Abrir Historial
                                            </a>

                                            @if($canDownloadClinicalPdf)
                                                <a href="{{ route('clinical_histories.pdf', $student) }}" class="inline-flex items-center justify-center px-4 py-2 rounded-full bg-emerald-600 text-white text-[11px] font-black uppercase tracking-widest hover:bg-emerald-700 transition w-full sm:w-auto">
                                                    PDF
                                                </a>
                                            @endif
                                        </div>
