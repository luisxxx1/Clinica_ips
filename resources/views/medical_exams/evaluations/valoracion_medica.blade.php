...
<div x-data="{
    ...
    <div x-show="tab === 'evaluacion'" class="space-y-8">
        ...
        <header class="mb-6">
            ...
            <p class="text-slate-400 font-medium">Paciente: <span class="text-emerald-700 font-bold">{{ $medical_exam->student->name ?? 'No especificado' }}</span></p>
        </header>
        ...
        <form action="{{ route('medical_exams.store_evaluation', $medical_exam) }}" method="POST" class="space-y-8">
            ...
            <div class="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm">
                <h3 class="text-emerald-700 font-black uppercase text-[10px] tracking-[0.2em] mb-6 flex items-center gap-3">
                    <span class="w-6 h-1 bg-emerald-600 rounded-full"></span> 1. Antropometría e IMC
                </h3>
                <div class="grid grid-cols-1 md:grid-cols-5 gap-6">
                    <div class="space-y-2">
                        <label class="block text-[10px] font-black uppercase text-slate-400 ml-2">Peso (kg)</label>
                        <input type="number" step="0.1" x-model="peso" name="results[peso]" class="w-full border-none bg-slate-50 rounded-2xl p-4 font-bold text-slate-700 focus:ring-2 focus:ring-emerald-500/20 shadow-sm" placeholder="0.0">
                    </div>
                    <div class="space-y-2">
                        <label class="block text-[10px] font-black uppercase text-slate-400 ml-2">Talla (cm)</label>
                        <input type="number" x-model="talla" name="results[talla]" class="w-full border-none bg-slate-50 rounded-2xl p-4 font-bold text-slate-700 focus:ring-2 focus:ring-emerald-500/20 shadow-sm" placeholder="0">
                    </div>
                    <div class="space-y-2">
                        <label class="block text-[10px] font-black uppercase text-slate-400 ml-2">IMC</label>
                        <div class="w-full bg-emerald-50 text-emerald-700 rounded-2xl p-4 font-black text-center border border-emerald-100" x-text="imc"></div>
                    </div>
                    <div class="space-y-2">
                        ...
                        <div class="w-full rounded-2xl p-4 font-black text-center text-[10px] tracking-widest uppercase border border-slate-100"
                            :class="imc > 0 ? 'bg-emerald-700 text-white' : 'bg-white text-slate-300'"
            ...
            <div class="bg-white p-8 md:p-10 rounded-[2.5rem] border border-slate-100 shadow-sm space-y-8">
                <h3 class="text-emerald-700 font-black uppercase text-[10px] tracking-[0.2em] mb-6 flex items-center gap-3">
                    <span class="w-6 h-1 bg-emerald-600 rounded-full"></span> 2. Antecedentes Médicos
                </h3>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-8">
                        ...
                        <div class="flex items-center justify-between p-4 bg-slate-50/50 rounded-2xl border border-transparent hover:border-slate-100 transition-all group">
                            ...
                            <div class="flex gap-4">
                                <label class="flex items-center gap-1 cursor-pointer">
                                    <input type="radio" name="results[{{ $key }}]" value="0" class="w-3 h-3 text-emerald-600 border-slate-300 focus:ring-0">
                                    <span class="text-[9px] font-black uppercase text-slate-400">No</span>
                                </label>
                                <label class="flex items-center gap-1 cursor-pointer">
                                    <input type="radio" name="results[{{ $key }}]" value="1" class="w-3 h-3 text-emerald-600 border-slate-300 focus:ring-0">
                    ...
                    <div class="space-y-6">
                        <div class="grid grid-cols-2 gap-3">
                            ...
                            <label class="flex items-center p-3 bg-white border border-slate-100 rounded-xl cursor-pointer hover:bg-emerald-50/50 transition-all group">
                                <input type="checkbox" name="results[antecedentes][]" value="{{ Str::slug($item) }}" class="rounded text-emerald-600 border-slate-200 focus:ring-0 w-3 h-3">
                                <span class="ml-2 text-[10px] font-bold text-slate-500 group-hover:text-emerald-700">{{ $item }}</span>
                            </label>
                            @endforeach
                        </div>

                        <div class="p-5 bg-emerald-50/30 rounded-[2rem] border border-emerald-100/50 space-y-4">
                            <select name="results[nacimiento]" class="w-full border-none bg-white rounded-xl text-xs font-bold text-slate-600 p-3 shadow-sm focus:ring-2 focus:ring-emerald-500/20">
                ...
                <div class="pt-4">
                    ...
                    <textarea name="notes" rows="3" class="w-full border-none bg-slate-50 rounded-[2rem] p-6 text-xs font-medium focus:ring-2 focus:ring-emerald-500/10" placeholder="Escriba detalles adicionales..."></textarea>
                </div>
            </div>

            <div class="flex justify-end">
                <button type="submit" class="bg-emerald-600 text-white px-12 py-5 rounded-3xl font-black uppercase text-[10px] tracking-widest shadow-2xl shadow-emerald-200 hover:scale-105 transition-all">
