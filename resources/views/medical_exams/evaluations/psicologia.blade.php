    ...
    <form action="{{ route('medical_exams.store_evaluation', $medical_exam) }}" method="POST" class="p-12 space-y-10">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div class="bg-slate-50/50 p-6 rounded-[2rem] border border-slate-100">
                    ...
                    <div class="flex flex-col gap-3">
                        <label class="flex items-center justify-between p-3 bg-white rounded-xl cursor-pointer hover:bg-purple-50 transition-all border border-transparent hover:border-purple-100 group">
                            <span class="text-xs font-bold text-slate-600 group-hover:text-purple-700">Adecuada</span>
                            <input type="radio" name="{{ $key }}" value="adecuada" class="text-purple-600 focus:ring-0 w-4 h-4 border-slate-200">
                        </label>
                        <label class="flex items-center justify-between p-3 bg-white rounded-xl cursor-pointer hover:bg-purple-50 transition-all border border-transparent hover:border-purple-100 group">
                            <span class="text-xs font-bold text-slate-600 group-hover:text-purple-700">Dispersa</span>
                            <input type="radio" name="{{ $key }}" value="dispersa" class="text-purple-600 focus:ring-0 w-4 h-4 border-slate-200">
                        </label>
                    </div>
                </div>
                @endforeach
        ...
        <section class="space-y-6">
            <div class="flex items-center gap-4 mb-4">
                <span class="w-10 h-10 bg-purple-50 text-purple-600 rounded-xl flex items-center justify-center font-black text-sm">02</span>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="space-y-2">
                    ...
                    <select name="aptitud_psicologica" class="w-full bg-slate-50 border-none rounded-2xl p-4 font-bold text-slate-700 focus:ring-2 focus:ring-purple-500/20 shadow-sm">
                ...
                <div class="space-y-2">
                    ...
                    <input type="text" name="pruebas" placeholder="Ej: Test de Bender" class="w-full bg-slate-50 border-none rounded-2xl p-4 font-bold text-slate-700 focus:ring-2 focus:ring-purple-500/20 shadow-sm">
                </div>
            </div>

            <div class="bg-slate-900 rounded-[2.5rem] p-8 mt-6">
                <label class="block text-[10px] font-black text-purple-400 uppercase mb-4 ml-2 tracking-widest">Observaciones y Diagnóstico</label>
                <textarea name="observations" rows="4" 
                    placeholder="Describa los hallazgos detallados..." 
                    class="w-full bg-slate-800 border-none rounded-2xl p-6 text-white placeholder:text-slate-500 focus:ring-2 focus:ring-purple-500/50 resize-none font-medium"></textarea>
            </div>
        </section>
